...
interface AdSenseInArticleProps {
  /**
   * Ad slot ID from Google AdSense (optional, uses default from NEXT_PUBLIC_GOOGLE_ADSENSE_IN_ARTICLE_SLOT if not provided)
   */
  adSlot?: string;
  /**
...
export function AdSenseInArticle({
  ...
  const finalAdSlot = adSlot || process.env.NEXT_PUBLIC_GOOGLE_ADSENSE_IN_ARTICLE_SLOT;
  
  // Don't render if no ad slot is available
  if (!finalAdSlot) {
    console.warn("AdSenseInArticle: No ad slot provided and NEXT_PUBLIC_GOOGLE_ADSENSE_IN_ARTICLE_SLOT is not set. Ad will not render.");
    return null;
  }
